...
- **Fase P1 (esquema, la principal).** Tabla `campos_protegidos` + RLS + sync + Dexie 12 + `historial.entidad_tipo` + `accesos_boveda.entidad_tipo` + `credenciales.tipo`. Sección "Seguridad" en la ficha y el formulario del dispositivo. **Un solo grupo de esquema**, como se hizo con N3, para una única intervención del usuario en Supabase.
- **Fase P2 (sin esquema).** Vínculo polimórfico en pasos y tareas, con normalización compatible hacia atrás. "Vincular información protegida" en el editor. Grafo, `ReferenciadoPor` e impacto antes de eliminar.
