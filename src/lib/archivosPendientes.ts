...
export async function subirOEncolarArchivo(referencia: string, archivo: Blob, nombre: string): Promise<ResultadoSubida> {
  if (!supabase || (typeof navigator !== 'undefined' && navigator.onLine === false)) {
    await encolarArchivo(referencia, archivo, nombre)
    return 'encolado'
  }

  const { error } = await supabase.storage.from('adjuntos').upload(referencia, archivo)
...
export async function encolarArchivo(referencia: string, archivo: Blob, nombre: string): Promise<void> {
...
async function subirUnoAStorage(referencia: string, contenido: Blob, tipo: string): Promise<{ message: string } | null> {
...
export async function procesarArchivosPendientes(subirUno: SubidorDeArchivos = subirUnoAStorage): Promise<void> {
  ...
  for (const archivo of pendientes) {
    const error = await subirUno(archivo.referencia, archivo.contenido, archivo.tipo)
    if (!error) {
