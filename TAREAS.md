...
**Despliegue confirmado (regla 14).** La tarea 270 (la agenda con ingresos, retiros y equipos liberados, y "¿Qué hace?" en las guías, commit `b8dfd13`) está servida en **https://soluciones-it-psi.vercel.app**: `/version.json` responde `b8dfd13`. Por contenido, sobre los 146 trozos que declara `/sw.js`: `Chasis-CVOggcwJ.js` (que calcula la agenda para el número de Resolver) contiene "sin equipo", "Liberado " y "Se retiró "; `SeccionesAgenda-DHaKzWkD.js`, las categorías nuevas con sus iconos; `ProveedorEjecucion-D5-gpjgk.js` (la ejecución de las guías), "¿Qué hace «"; `TarjetaComando-zrH2EYCN.js`, la variante sin título que usa la hoja. **En negativo:** ningún trozo contiene "sin equipo asignado", "Disponible para asignar" ni "sigue a su nombre" (los textos largos que se acortaron antes de subir). **Sin cambios de esquema:** no hay que ejecutar SQL por esta tarea.

**Despliegue confirmado (regla 14).** La tarea 269 (Diagnóstico dentro de Resolver y fuera de Más, commit `d403afd`) está servida en **https://soluciones-it-psi.vercel.app**: `/version.json` responde `d403afd`. Por contenido, sobre los 149 trozos que declara `/sw.js`: `SolucionesPage-DySVZXhr.js` contiene "Diagnósticos: crear, editar, estadísticas y sugerencias"; `DiagnosticosPage-6_t9vTP6.js`, "Diagnósticos: empezar por el problema"; `origenNavegacion`, `/diagnostico` con padre Guías y la administración rotulada "Guías con preguntas". **En negativo:** ningún trozo contiene ya "Diagnóstico inteligente" ni "Del síntoma a la guía, paso a paso" (la fila de Más). **Sin cambios de esquema:** no hay que ejecutar SQL.
...
**Despliegue confirmado (regla 14).** La tarea 257 (Más e Infraestructura, commits `c546b7c`, `9fefd04` y `efa502a`) está servida en **https://soluciones-it-psi.vercel.app**: `/version.json` responde `efa502a`. Por contenido, sobre los 139 trozos que declara `/sw.js`: `PantallaMas-BIeJLDKl.js` contiene "Infraestructura", "El mapa completo, desde cada raíz" y "Lo que marcaste con la estrella", y "Mejor desde el ordenador" como nota de fila; `iconos-DEb5c8q0.js`, el trazo del icono nuevo `Graph`; "Actividad del equipo" está solo en `AgendaPage-CIkUBWqS.js`, ya no en el de Más. Las tres primeras y el trazo no existían antes de la tarea (comprobado sobre `0fc6e5d`). **En negativo:** ningún trozo contiene ya "Trabajo técnico", "Lo mío y lo del equipo" ni "Se puede hacer aquí, pero pide teclado y pantalla grande.". **Recordatorio:** PWA con `registerType: 'prompt'`: en un teléfono con la app instalada hay que aceptar "Versión nueva disponible" (o Más > Buscar actualización). **Sin cambios de esquema:** no hay que ejecutar SQL.

**Despliegue confirmado (regla 14).** La tarea 263 (Resolución guiada, commits `551d4b7`, `802ffc2` y `a1f6e68`) está servida en **https://soluciones-it-psi.vercel.app**: `/version.json` responde `a1f6e68`. Por contenido, sobre los 139 trozos que declara `/sw.js`: "Guía con preguntas" está en `favoritos-BpQyPw68.js`, `diagnostico-DXYjdV3E.js` y `ResolverPage-Dy9MpLFN.js`; "Resolviendo" y "Te lleva a", en `DiagnosticoRunPage-BZBvafim.js`; "Hay que escalar", en `diagnostico-DXYjdV3E.js` y `DiagnosticoRunPage-BZBvafim.js`. Ninguna de las cuatro existía antes de la tarea. **En negativo:** ningún trozo contiene ya "Inicio del diagnóstico", el rótulo que el editor cambia por "Primera pregunta". **Recordatorio:** PWA con `registerType: 'prompt'`: en un teléfono con la app instalada hay que aceptar "Versión nueva disponible" (o Más > Buscar actualización). **Sin cambios de esquema:** no hay que ejecutar SQL. **Antes de rellenar "Cómo termina" en los diagnósticos, actualizar los teléfonos:** una copia anterior que edite y guarde un diagnóstico descarta ese campo.

**Despliegue confirmado (regla 14).** La tarea 256 (Equipos + QR, commits `6b8e756`, `c4597b5` y `e37121d`) está servida en **https://soluciones-it-psi.vercel.app**: `/version.json` responde `e37121d`. Por contenido, sobre los 139 trozos que declara `/sw.js`: `DispositivosPage-BOuQB0DV.js` contiene "Escanear QR", "Crear equipo", "Equipos de red" y "Agregarlos desde «Crear equipo»"; `DispositivoPage-BuEdwcKt.js`, "Conectado a", "Ver conexión" y "Más datos del equipo"; `EscanerPage-rdzewT_-.js`, "Código para conectar un computador" y la clave `escaner:ultimo-abierto`. **En negativo:** ningún trozo contiene ya "Equipo identificado", "La ficha vuelve aquí al terminar" ni el menú "Más acciones: ubicaciones, personas, etiquetas QR, importar". **Recordatorio:** PWA con `registerType: 'prompt'`: en un teléfono con la app instalada hay que aceptar "Versión nueva disponible" (o Más > Buscar actualización). **Sin cambios de esquema:** no hay que ejecutar SQL.

**Despliegue confirmado (regla 14).** La tarea 255 (ejecución visual de las guías, commits `8d656b6`, `aebd189` y `b0c49b3`) está servida en **https://soluciones-it-psi.vercel.app**: `/version.json` responde `b0c49b3`. Por contenido, sobre los 139 trozos que declara `/sw.js`: `AsistenteVista-C5mmsFl6.js` contiene "Ruta del procedimiento", "Ver la ruta completa", "Qué hacer", "Ten esto listo antes de empezar." y "La ruta: "; `ProveedorEjecucion-CssplcxY.js`, "Debes ver" y "Credencial necesaria"; `ArticuloForm-DyBVqxRc.js`, "Para qué: qué se logra con este paso", "Dónde se hace: menú, ventana o sección", "Debes ver: qué aparece al terminar" y "Sacar de «Requisitos»"; `Historial-BhpsDqBW.js`, "qué debe verse al terminar el". **En negativo:** ningún trozo contiene ya "Antes de empezar, ten a mano", "Objetivo: qué se logra al terminar", "Antes de empezar (un requisito por línea)" ni "Sacar de «Antes de empezar»". **Recordatorio:** PWA con `registerType: 'prompt'`: en un teléfono con la app instalada hay que aceptar "Versión nueva disponible" (o Más > Buscar actualización). **Sin cambios de esquema:** no hay que ejecutar SQL. **Antes de rellenar "Dónde" y "Debes ver" en las guías, actualizar los teléfonos:** una copia anterior que edite y guarde una guía descarta los dos campos.

**Despliegue confirmado (regla 14).** La tarea 254 (Resolver y navegación de cuatro destinos, commits `c27d74d`, `f229493` y `fd25039`) está servida en **https://soluciones-it-psi.vercel.app**: `/version.json` responde `fd25039`. Por contenido, sobre los trozos que declara `/sw.js`: `ResolverPage-C2OuTlJY.js` contiene "¿Qué necesitas resolver?", "Buscar problema, equipo, comando", "Accesos rápidos", "Todas las guías", "Vas en el paso" y "Ver la agenda completa"; `Chasis-CEQt-wt-.js` contiene "Resolver" y "Navegación principal". **En negativo:** ningún trozo se llama ya `InicioPage` y el chasis ya no contiene "Trabajo técnico". **Recordatorio:** PWA con `registerType: 'prompt'`: en un teléfono con la app instalada hay que aceptar "Versión nueva disponible" (o Más > Buscar actualización). **Sin cambios de esquema:** no hay que ejecutar SQL.
...
*(vacío: las tareas **244** y **246** (resolver rápido con guías, encargo del 2026-09-17, y su segunda pasada del mismo día) se cerraron ese día y están en [TAREAS_ARCHIVO.md](TAREAS_ARCHIVO.md). La tarea **252** ya aplicó a la guía DIAN las correcciones editoriales que señala el editor. La **245** continúa en "Por hacer" únicamente por las nueve confirmaciones técnicas que requieren FrontRest, HKA Factura y una ejecución real del procedimiento.)*
...
- **En negativo:** **ningún** chunk contiene ya "Anotar los requisitos previos", "no se puede ejecutar", "Se leen aquí en cualquier momento" ni "Precaución, dato o consejo".
...
- `ProcedimientoVista-o43ZAX3j.js` contiene "Sí, lo comprobé" (la verificación con sus dos controles en la vista de paso entero).
- `Chasis-Qe08Qzx5.js` contiene el cuerpo nuevo de `descripcionVencida`, que delega el conteo en el ayudante de días de calendario y devuelve "Venció" cuando la fecha no es legible. Es la prueba de que el cambio 6 entró.

**Recordatorio para el equipo:** la app es una PWA con `registerType: 'prompt'`. En un teléfono que ya la tiene instalada, la versión nueva NO se activa sola: aparece el aviso "Actualización disponible" y hay que aceptarlo. En escritorio, recarga forzada. **Esta versión no cambia el esquema:** no hay que ejecutar SQL en Supabase.

**Despliegue confirmado (regla 14).** El commit `6a60ea5` (tarea 233, informe del 2026-09-08) está servido en **https://soluciones-it-psi.vercel.app**. Comprobado **por contenido** sobre los chunks reales que declara `/sw.js`:

- **En positivo:** `AsistenteVista-BeI7aQHL.js` contiene "Primero, completa esta guía", "Del paso (", "Sí, lo comprobé", "Detenerme aquí y dejarlo anotado" y "Al terminar se comprueba"; `ArticuloForm-BtLAYP1b.js` contiene "Añadir contenido a esta tarea" y "Sin asignar"; `SolucionesPage-CB9tvEU4.js` contiene "Todas las categorías".
- **En negativo**, que es lo que prueba que el cambio entró: ningún chunk contiene ya "Termina el procedimiento vinculado para poder avanzar", "para poder avanzar" ni "Sin terminar".
...
- `BarraTarea-BRZekg6o.js` contiene tanto "vuelves aquí al terminar" (la rama normal, que siguen usando las otras 13 pantallas de nivel tarea) como el marcado de la rama `compacta` (`h-11 items-center gap-1`): las dos ramas conviven en el mismo chunk compartido, como se espera de un cambio opt-in.
...
- **Avance verificado el 2026-09-21 (tarea 252):** se abrió la guía real con una sesión autorizada, se revisó completa, se dividieron las acciones encadenadas con las ayudas del editor, se reescribió "Antes de empezar" como condiciones previas, se agregó un tiempo estimado de 30 minutos y se guardó el motivo del cambio. El editor marca **100 % de completitud** y la sincronización llegó a Supabase. Buscar "DIAN" desde Inicio muestra la guía primero y un toque abre directamente el paso 1, no el editor. La guía continúa en `borrador` porque las nueve confirmaciones operativas de abajo siguen abiertas; completitud editorial no significa validación técnica.
...
- **Causa de "la guía no aparece" (encontrada el 2026-09-20, tarea 248).** La guía **sí existe**: id `a1ac8d0a-72e7-4dd1-a377-afd2a2ca1cc0`, categoría POS, nueve pasos, no eliminada, **estado `borrador`**. El buscador global solo indexa lo publicado (correcto), y el aviso de "hay borradores que coinciden" solo se dibujaba dentro del estado "Sin coincidencias": como buscar "DIAN" encuentra la ficha **HKA Factura**, había resultados y el aviso no salía nunca. Corregido en la tarea 248: ahora la guía se ve en Inicio, en el bloque **"Borradores coincidentes"**, con "Revisar borrador" al editor. **Eso NO cierra esta tarea:** la guía sigue en borrador y su contenido sigue sin revisar.
- **Las nueve confirmaciones críticas que siguen ABIERTAS (2026-09-20).** Ninguna se puede resolver desde esta sesión: todas exigen mirar FrontRest o HKA en vivo, o ejecutar el procedimiento. **No se sustituye ninguna por una suposición** (regla 20d).
  1. Ruta exacta de la versión ACTUAL de FrontRest hasta la configuración de resoluciones (confirmar viéndola, no de memoria).
  2. Qué valor del **Formato 1876** corresponde exactamente al campo "Número de resolución" de FrontRest.
  3. Nombre exacto del campo de **fecha inicial** en FrontRest.
  4. Mecanismo exacto para **desactivar la resolución anterior y activar la nueva** en FrontRest.
  5. Si **HKA Factura completa solo** el valor inicial del rango.
  6. Valores internos de la organización en HKA: **ambiente, tipo de documento, tipo de operación, establecimiento, tipo de servicio y modalidad de uso**.
  7. Ejecutar el procedimiento **completo en el teléfono** (o en la vista móvil) de principio a fin.
  8. Comprobar que **FrontRest, HKA Factura y el Formato 1876** quedan con los mismos valores.
  9. Comprobar que el **secuencial anterior correcto queda inactivo**, sabiendo que HKA avisa de que no se puede reactivar.
- **Condición de publicación:** solo cuando las nueve estén resueltas se publica desde el editor de la app (nunca por SQL) y se espera a que termine la sincronización; después se comprueba en Supabase que la fila tenga `estado = publicado`, `eliminado_en = null` y el mismo id. Mientras tanto **se queda en `borrador`**, que es lo correcto: un procedimiento de facturación a medias no puede presentarse como oficial.
- **Prioridad:** Alta. **Estado:** Pendiente y BLOQUEADA. **No se cierra** mientras quede alguna de las nueve confirmaciones de abajo: cada una puede afectar la facturación, la activación de la resolución nueva o la inactivación del secuencial anterior.
...
**Lo que era.** El service worker sirve **todas las navegaciones desde su propio `index.html` precacheado** (`NavigationRoute` + `createHandlerBoundToURL`, `vite.config.ts`). Si a ese build le falta un trozo en la caché (Android desaloja cachés cuando aprieta el almacenamiento) la app lo pide a la red, y ahí ya no está: **cada despliegue retira los assets del anterior**, y el 2026-09-03 y 04 hubo cinco seguidos. El `ErrorBoundary` detectaba el fallo de trozo y recargaba una vez (`recargarUnaVezPorChunk`), pero esa recarga vuelve a leer el MISMO `index.html` roto de la caché: falla igual, y el botón "Recargar" repetía exactamente el intento que ya había fracasado. **Ese era el bucle: un callejón sin salida, no un fallo nuevo.** Estaba latente desde que existe la PWA; los cinco despliegues seguidos solo lo dispararon.

**La corrección.** `src/lib/recargaChunk.ts` gana `reinstalarYRecargar()`: da de baja los service workers, borra las cachés del navegador y recarga. **No toca IndexedDB**, y es deliberado: ahí viven el avance de los procedimientos, los favoritos, la cola de subida y la bóveda. Se borra solo lo que se puede volver a bajar. El `ErrorBoundary` lo encadena como **segundo** intento automático (recargar, y si vuelve a fallar, reinstalar), con un freno de 60 s para no gastar los datos del técnico si el problema no era la caché. El botón pasa a decir **"Reinstalar la aplicación"** y hace eso, con la promesa escrita debajo de que sus guías y su avance no se tocan.

**Lo que queda por hacer, y no se hizo aquí:** espaciar los despliegues o conservar los assets de la build anterior en Vercel reduciría la exposición de raíz. Hoy la app se recupera sola, que es lo urgente, pero sigue habiendo una ventana en la que un teléfono con la caché desalojada tiene que volver a bajarlo todo. Si el equipo lo vuelve a reportar, esa es la siguiente palanca.
