...
- **Qué cambia:** `Chasis` (modo `'tarea'`) y `BarraTarea` ganan una prop `compacta?: boolean`. Con ella, la cabecera de nivel tarea deja de ser el bloque de 68 px con rótulo ("Editando"/"Ejecutando"), título y ruta de vuelta en tres líneas, y pasa a una **sola línea de 44 px**: la X, el título truncado y `children` (lo que se porte a la ranura, `barra` o `BandaTarea`) en la MISMA fila, en vez de en un bloque debajo.
- **Alcance deliberadamente acotado:** el prop es opt-in y por defecto `false`, así que los otros 13 usos de `Chasis modo="tarea"` (los editores de artículo, dispositivo, credencial, persona, ubicación, diagnóstico, y las pantallas de reemplazo/baja/importación/migración) **no cambian**. Solo `AsistentePage` la activa hoy.
