...
| `BTN_ICONO_SECUNDARIO` | cuadrado 44x44 delineado (34x34 hasta la tarea 262) |
| `BTN_ICONO_PELIGRO` | cuadrado 44x44 sin borde, rojo (34x34 hasta la tarea 262) |

**Toque de 44 en los botones de icono (tarea 262, regla R6).**
- **Caja y dibujo:** la caja entera es el objetivo y mide 44x44. El dibujo lo pone cada consumidor, de 16 a 18 px, y no crece.
- **Por qué la caja visible y no un área invisible alrededor de un cuadrado de 34:** con los 6 px entre iconos vecinos, las áreas se solaparían y un toque entre dos caería en cualquiera de ellos.
- **Controles sin borde que no deben mover lo de al lado** (la X de un modal): la caja de 44 lleva el margen negativo que le devuelve la huella del dibujo, `-m-[13px] h-11 w-11` con una X de 18.
...
- **Franja del aviso de versión nueva (tarea 274).** En los niveles con pestañas, lo último de la columna de contenido es un hueco para `AvisoActualizacion` (`huecoAvisoActualizacionChasis`): pegajoso sobre las pestañas en el teléfono y al borde desde `md`, con el fondo y el borde de la barra de pestañas. Está en el flujo, así que reserva su sitio: al final del scroll no deja nada debajo del aviso y, al aparecer, no mueve lo de arriba. Vacío no ocupa nada (`empty:hidden`). Una barra de acciones de la pantalla que publique su propio hueco (la de una guía o la de la ficha de un equipo) manda sobre la franja.
...
- **La X del modal (tarea 262):** caja de 44x44 con la X de 18 y `-m-[13px]`, que le devuelve la huella de 18 de antes. Con eso el título no se mueve, y la caja crece sobre el relleno de 20 del modal. Antes medía 26x26.
...
- **Variantes:** `'cabecera'` usa `BTN_ICONO_SECUNDARIO` (botón con borde, fila de acciones de una ficha; 44x44 desde la tarea 262); `'fila'` es solo el icono con tinte al pasar (fila de lista; sigue en 34x34, fuera del alcance de la 262). El color activo va sobre el SVG.
...
- **Propósito:** saber, cuando uno quiere, si el teléfono tiene la versión desplegada, y poder decir cuál lleva. `BuscarActualizacion` es el bloque "Aplicación" de Ajustes (fila de Más hasta la tarea 268); `AvisoActualizacion` es el aviso "Versión nueva disponible" con su botón: una fila dentro de la barra de acciones de la pantalla (una guía en curso, tarea 273) o de la franja del chasis (tarea 274), y la pastilla flotante donde no hay ninguna.
...
- **`AvisoActualizacion`:** `{ visible, onActualizar }`. Al tocar, pasa a "Actualizando..." y se deshabilita. El botón mide 44 px de alto (`min-h-11` solo en él, tarea 262; `BTN_PRIMARIO` no cambia), así que la fila del aviso mide 58 px y la pastilla ocupa de 80 a 142 px del borde. Sale con `needRefresh` de la librería **o** con la fase `disponible` que descubre una comprobación propia: `needRefresh` solo se enciende si el evento llega a esa ventana, y con la PWA instalada el worker puede quedar en espera sin que se vea. **Dónde se pinta (tarea 273):** la pastilla (`fixed bottom-20 z-50`) quedaba encima de la barra de acciones de una guía, así que esa barra publica un hueco y el aviso se pinta en él por portal, como fila encima de los botones y en el flujo de la barra: no tapa nada, ningún control cambia de sitio y vacío no ocupa espacio (`empty:hidden`). El hueco vive en `ranuraAvisoActualizacion.ts` (`huecoAvisoActualizacion` es el `ref` del hueco; `useHuecoAvisoActualizacion`, lo que lee el aviso) y es de módulo, no de contexto, porque el aviso se monta en la raíz de la app, fuera de la guía. **Fuera de una guía (tarea 274):** la pastilla tapaba el final de las pantallas normales (los accesos rápidos de Resolver), así que el chasis publica también su franja (`huecoAvisoActualizacionChasis`, ver 2.0), con menos rango: si una barra de la pantalla publica su hueco, el aviso va en ella. **En las pantallas de tarea (tarea 275):** no tienen franja del chasis, así que toda barra de acciones propia que llegue a la altura de la pastilla (80 px del borde) publica el hueco: hoy la del editor de guías (`ArticuloForm`) y la de asignar un equipo (`AsignarEquipoPage`). Las de una sola fila (editores de equipo, de diagnóstico y de credencial, Importar, Etiquetas) miden de 65 a 71 px y no la necesitan. Donde no hay ningún hueco (el inicio de sesión, una tarea sin barra que llegue a su altura), la pastilla de siempre.
- **Dónde:** `CuentaPage` (Ajustes > Aplicación, desde la tarea 268; antes `PantallaMas`) y `ActualizacionDisponible` (global). Los huecos del aviso, en las barras de acciones de `AsistenteVista` (nivel 0, y la fila en línea de una guía vinculada que ocupa la pantalla), de `ModoFoco` (no anidada), de la ficha de un equipo (`DispositivoPage`), del editor de guías (`ArticuloForm`) y de asignar un equipo (`AsignarEquipoPage`), y la franja del `Chasis`.
...
- **Personas y equipos (tarea 270):** tres categorías más (`persona_ingreso` y `persona_retirada` con el icono `User`, `equipo_liberado` con `Monitor`, que es "equipo"); "Revisar" para el equipo liberado y la persona retirada. `FilaAgenda` solo añade el origen cuando lo hay (las filas de personas no lo repiten). Los asuntos los calcula `inicio/asuntosDePersonas.ts` (probado), que `calcularPendientes` junta con los demás.

### 3.8s-ter `referencia/QueHaceEnTexto` y `referencia/comandosEnTexto.ts` (2026-09-23, tarea 270)
- **Propósito:** "¿Qué hace?" para los comandos y atajos que una instrucción ESCRIBE y que tienen ficha en el Centro de consulta, sin que el autor la enlazara (RN-055).
- **`comandosEnTexto(texto, referencias)`:** las fichas de comando o atajo cuyo valor aparece entero en el texto (mayúsculas, espacios y el "+" de un atajo no cuentan; un hueco como `[dirección]` vale por una palabra y los del final son opcionales; bordes de palabra; el más largo gana; sin duplicados; tres como mucho). `fichasEnlazadasDelPaso(bloques)` da los ids que el paso ya enlaza, para no repetirlos.
- **`QueHaceEnTexto`:** `{ texto, referencias, excluir?, className? }`. Una `ChipReferencia` con `rotulo` "¿Qué hace «valor»?" por hallazgo; nada si no hay. No consulta la base: recibe el mapa vivo (`useReferencias`) de quien lo monta.
- **Dónde:** `ModoFoco` (bajo las etiquetas de términos de la acción) y `BloqueVista` (debajo de la tarea, alineado con su texto, cuando quien la monta pasa `referencias`: `ProcedimientoVista` y `AsistenteVista`, con una sola consulta para todo el documento).
- **Cambios en lo que reutiliza:** `ChipReferencia` acepta `rotulo` (texto visible y nombre accesible) y se toca en 44 px con su pastilla de 36; `HojaReferencia` pinta `TarjetaComando` para un comando o un atajo, y `TarjetaComando` acepta `sinTitulo` para no repetir el título de la hoja.

### 3.8aa Asistencia remota: `asistencia/*` y el portal `src/asistencia/*` (2026-09-24, tarea 258)
- **`asistencia/modelo.ts`** (sin importar nada de la app): el formato de un envío (`ContenidoAsistencia` v1 con `titulo`, `subtitulo` y hasta 40 `BloqueAsistencia` de 10 tipos), `validarContenido` y `pareceSecreto` (espejo exacto de `asistencia_validar_contenido` y `asistencia_parece_secreto` del servidor; `modelo.test.ts` compara las dos listas) y `formatoCodigo` ("482 731").
- **`asistencia/contenidoPaso.ts`** · `construirContenidoDePaso({ paso, numeroPaso, tituloGuia, referencias, tareaId? })`: arma el envío de un paso (o de una acción) solo con campos permitidos; nunca lee `vinculoProtegido`; devuelve `{ contenido, apartados }` con lo que no se envía (`secreto`, `largo`, `limite`). `urlsEnTexto(texto)`.
- **`asistencia/VistaContenidoAsistencia`** · `{ contenido, recibido?, destacado? }`: UN SOLO componente para el portal y para la vista previa del técnico. Todo como texto; reutiliza `DondeSeHacePaso` y `DebesVerPaso` (`SenalesDePaso`) y los tonos de `TONOS_AVISO`; comandos, datos, archivos y enlaces con Copiar de 44 px; un tipo desconocido no se dibuja.
- **`asistencia/HojaEnviarAEquipo`** · `{ abierto, onCerrar, onEnviado, sesion, paso, numeroPaso, tituloGuia, referencias, tareaId }`: la vista previa obligatoria sobre `Modal`, con acciones fijas al pie.
- **`asistencia/FranjaEquipoConectado`** · `{ paso, numeroPaso, tituloGuia, referencias, tareaId }`: indicador y acción en 44 px; no pinta nada sin conexión; monta el latido. La montan `ModoFoco` (prop nueva `renderEnvioAEquipo`, solo en la guía principal) y el pie de la vista de paso entero de `AsistenteVista`.
- **`asistencia/ConectarPage`**: la pantalla `/conectar`. **`asistencia/sesionAsistencia.ts`**: la conexión del técnico (localStorage por usuario, sincronizada entre pestañas), `useSesionAsistencia`, `useLatidoAsistencia`, `conectarEquipo`, `enviarAlEquipo`, `desconectarEquipo`, `desconectarAlSalir`. **`asistencia/apiTecnico.ts`**: las cuatro RPC del técnico y sus textos de error y de cierre.
- **Portal (`src/asistencia/`)**: `main.tsx` (entrada propia), `PortalAsistencia` (estados y consulta adaptativa), `apiPortal.ts` (`fetch` a las tres RPC anónimas, sin el cliente de Supabase), `estadoPortal.ts` (lógica pura: intervalos, textos de cierre, sesión de la pestaña, `unirMensajes`), `CodigoQr` y `qr.ts` (el QR como SVG de React, sin `innerHTML`). `aislamiento.test.ts` falla si el portal importa la app; `scripts/verificar-portal.mjs` comprueba lo mismo sobre el build.
- **`HojaPasos`** suma `rutaConectar` y `estadoConectar` ("Conectar un equipo", solo sin equipo conectado).
