...
- **Los apoyos del paso, UNA sola vez (tarea 234, sección 3 del encargo del 2026-09-09).** Dónde se pintan lo decide `ubicacionApoyosDelPaso` (en `apoyosTarea.ts`), que devuelve **un solo destino de tres**: `'sueltos'` (pegados a la instrucción, al entrar al paso), `'panel'` (dentro del control "Del paso") o `'ninguno'` (en las tareas siguientes). Antes la condición de los sueltos incluía "o el panel está abierto", y el panel pinta esa misma lista: abrirlo mostraba la precaución **dos veces**. Al ser los tres destinos excluyentes, la duplicación deja de ser representable. El chip "Del paso" se muestra **también en la primera tarea**: ahí solo se ven sueltos los avisos, así que la galería del paso, sus imágenes y su dato protegido no tenían control que los abriera.
