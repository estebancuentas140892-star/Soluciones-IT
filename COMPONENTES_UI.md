...
- **Quién lo escribe:** el que ORIGINA el salto, nunca el destino (el destino no puede saber de dónde lo abrieron). Hoy: `EscanerPage` (`Escáner`), `TopologiaEquipoPage` (`Topología`) y `DispositivoPage` (el nombre del equipo, para los enlaces a ubicación, persona y reemplazos).
- **Quién lo consume:** solo el `Chasis`, en `documento` (regreso, etiqueta y línea de contexto) y en `tarea` (destino de la X y ruta de vuelta escrita). Ninguna pantalla lo lee por su cuenta.
...
- **Propósito:** tarjeta de un artículo en un listado, en **tres zonas apiladas**: título, metadatos y acción. Reemplaza el marcado que `SolucionesPage` y `CategoriaPage` copiaban por separado.
...
- **Línea de capacidad (tarea 214, tablero `3b`).** La fila decía `categoría · tipo · min`, exactamente igual para una guía de 7 pasos con verificación final que para un borrador sin un solo paso, así que el técnico descubría que la guía estaba vacía **después de abrirla**, de pie y frente al equipo. Ahora dice **"7 pasos · ~25 min · verificación"** o **"Sin pasos · solo notas · no se puede ejecutar"** (ver `capacidadGuia.ts`, 3.8k).
- **La acción dice cuál de las tres cosas hace.** Guía ejecutable sin avance: **"Empezar"** a ancho completo y 48 px, directo a `/ejecutar`. Con avance a medias (`avance`, leído de `progresoPasos` por `SolucionesPage`): **"Continuar · paso N de M"**, el mismo criterio que `BarraAccionFicha`, para que la tarjeta y la ficha no se contradigan. Sin procedimiento: **"Abrir"** de 44 px, al ancho justo y a la derecha, porque en un catálogo con más notas que guías darle el mismo peso llenaría la pantalla de botones que solo repiten el enlace del título.
- **`avance` no resucita "Sin terminar"** (retirado por H01): no ordena, no filtra ni saca ninguna guía de su sitio; solo cambia el rótulo de la que ya estás mirando. Con 0 pasos hechos, o con todos, la acción vuelve a decir "Empezar".
- **No es un enlace que envuelva a la acción**: un control dentro de otro control no es HTML válido y el lector de pantalla no sabría cuál anuncia. El cuerpo (zonas 1 y 2) es el enlace y la acción vive en su propia fila, con `aria-label` que nombra la guía y, al continuar, el paso.
...
- **Exporta cuatro piezas**, compartidas por la ficha real (`ArticuloPage`) y la prueba del editor (`VistaPreviaArticulo`), para que el autor vea exactamente lo mismo que el técnico:
  ...
  - **`IntroduccionGuia({ procedimiento })`**: **Objetivo** y **Antes de empezar** (los requisitos), cada uno en su sección corta. Vivían dentro de `ProcedimientoVista`, así que desaparecían al quitarlo de la ficha.
...
- **Dónde:** `ArticuloPage` (ficha) y `VistaPreviaArticulo` (prueba del editor, que ahora entra por la presentación y abre el procedimiento con **"Empecemos"**).
...
- **Medidas que lo definen:** instrucción a **30 px** (lo que se lee de brazo estirado, a pleno sol, con el teléfono apoyado en el rack) y botón grande de **76 px**, el ÚNICO elemento grande de la pantalla, así que no hay que apuntar. Chips de vínculos a 52 px, flechas de tarea 64x56, "Falla" 56.
- **Comportamiento:** entra en el primer elemento pendiente del recorrido. La acción dominante **registra la tarea Y trae la siguiente** en un solo gesto; **desmarcar no mueve nada**, porque quien desmarca se está corrigiendo. Los vínculos (clave, fotos, archivos) se despliegan como chips, sin ocupar la pantalla.
- **UNA SOLA ACCIÓN PARA CONTINUAR (encargo del 2026-09-10, tarea 5).** Había dos controles que lo parecían, "Marcar hecha" y la **flecha derecha**, y solo uno registraba trabajo: con la flecha se recorría el paso entero sin marcar nada y el cierre aparecía bloqueado al final. **La flecha derecha se retiró.** El rótulo del botón grande dice ahora las dos cosas que hace: **"Hecho · continuar"** en una acción y **"Sí, lo comprobé · continuar"** en una comprobación; un aviso conserva su "Entendido · continuar"; una decisión, sus dos respuestas; y una entrada `guia-del-paso` **no dibuja botón**, porque su acción dominante es la de la tarjeta del vínculo. Completar la última tarea del paso **cierra el paso y abre el siguiente** sin un segundo clic (lo hace `alternarTarea` del hook, que encadena `intentarCompletarPaso`).
- **Consultar una tarea ya cumplida** (se llegó con el regreso) muestra la pastilla **"Completada"**, la acción dominante pasa a **"Continuar"** y **"Desmarcar esta tarea"** queda al pie como secundaria. Antes el botón grande era el propio "Hecha", que al tocarlo desmarcaba: el gesto de mayor superficie deshacía trabajo.
- **`onFalla` se ofrece como "Tengo un problema"**, en la fila secundaria y ya no entre dos flechas. Abre la misma hoja de salidas **sin completar nada ni cambiar de tarea**. Antes se llamaba "Falla" y en una comprobación cambiaba a "No se cumple": dos rótulos para el mismo control.
- **Los avisos son elementos del recorrido (encargo del 2026-09-10, tarea 1).** `tareasParaFoco` intercala una entrada de clase `'aviso'` por cada bloque: el del **paso** una sola vez y delante de todo, el de una **tarea** justo antes de esa tarea. Ocupa el área principal él solo, con icono, palabra, barra lateral y fondo de su tono (nunca solo el color, regla **R16**), y su botón dice **"Entendido · continuar"** en precaución o importante y **"Continuar"** en el resto. Confirmarlo avanza pero **no cuenta como tarea hecha**: la confirmación vive en `avisosConfirmados`, estado de `AsistenteVista` (props `avisosConfirmados` y `onConfirmarAviso`), no en el avance guardado, así que repetir la guía los vuelve a mostrar y volver atrás dentro de la misma ejecución no los vuelve a exigir.
...
- **Un paso SIN tareas ya no expulsa a nadie** (tarea 217, hallazgo G-18): `tareasParaFoco` lo presenta como **una sola tarea** con el título del paso a los mismos 30 px, y el botón grande cierra el paso directamente. Esa pseudo tarea lleva el id `paso:<id>`, que no puede chocar con el de ningún bloque, y no monta ni la pastilla "Tarea N de M" ni los segmentos: un solo segmento no mide nada.
...
- **Los apoyos del paso, UNA sola vez.** El reparto por `alcance`/`tareaId` vive en `apoyosTarea.ts` y hay **un solo contenedor** para lo que es del paso: el panel que abre el chip "Información del paso". Desde el **2026-09-10** ese panel ya **no lleva avisos** (son elementos del recorrido, ver arriba) y se queda con las imágenes, los archivos y el dato protegido del paso.
...
- **Props:** `{ abierto, onCerrar, resumenes: ResumenPaso[], subtitulo, onIrAPaso, modoEjecucion, onCambiarModo }`.
