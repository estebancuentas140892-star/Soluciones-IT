...
- **Medidas fijas:** 46 px de alto, lupa de 18 que se tiñe de acento al escribir, borde de acento mientras hay texto **y mientras el campo tiene el foco** (`focus-within`, segunda pasada del 2026-09-17; el input lleva `data-campo-busqueda` para que el anillo global de `:focus-visible` no dibuje un segundo marco rectangular dentro de la caja, regla en `src/index.css`), y **borrar de 44x44 con margen negativo** (antes unos 26 px en todas menos Guías, y es el control que más se falla porque se usa con el teclado abierto). La "x" nativa de WebKit se oculta siempre, porque duplicaba el botón y medía la mitad.
...
- **Props:** `{ titulo: string, conLupa?: boolean, volverEnMovilA?: string, children?: ReactNode }`. `children` es la banda de controles propios de la pantalla, que se dibuja justo debajo dentro del mismo bloque pegajoso. **`volverEnMovilA` (2026-09-17):** chevron de regreso de 44 px antes del título, **solo en móvil** (`md:hidden`); lo pasa el chasis en Equipos, Red y Bóveda, que en el teléfono dejaron de ser pestañas y se abren desde Más.
