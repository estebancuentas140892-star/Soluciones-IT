...
19. Política de mantenimiento de tareas y documentación (acordada con el usuario el 2026-07-23). Es OBLIGATORIA para cualquier modificación del proyecto, sin importar su tamaño. No puede existir ningún cambio sin documentación ni ninguna tarea sin seguimiento.
    ...
    - **(b) DOCUMENTACION_FUNCIONAL.md al día.** Se trata como parte del código fuente (única fuente de verdad del funcionamiento). Se actualiza EN LA MISMA TAREA que el cambio de código, nunca después, ante cualquiera de: alta/baja/modificación de pantalla, formulario, campo, botón, validación, valor por defecto, dependencia de campo, flujo, navegación, menú, permiso, lógica de negocio, estructura de datos, componente (nuevo/eliminado/modificado/reutilizado) o comportamiento de un módulo. Nunca se permite que quede desactualizada respecto al código.
    - **(c) Historial de cambios.** Cada modificación se anota en la sección "Historial de cambios" de DOCUMENTACION_FUNCIONAL.md con: **Fecha, Área modificada, Tipo (Agregado/Modificado/Eliminado/Refactorizado/Optimizado), Descripción, Motivo, Impacto esperado**. Si el cambio afecta la estructura de datos, documentar además tablas, campos, relaciones, restricciones, validaciones e impacto sobre otros módulos.
    - **(d) Verificación cruzada y finalización.** Al cerrar cualquier tarea se verifica que el código coincide con la documentación, que no hay funcionalidades sin documentar ni elementos documentados que ya no existan, y que campos/botones/flujos están al día. Ninguna tarea se considera COMPLETADA hasta que: el código funcione, TAREAS.md esté actualizado, DOCUMENTACION_FUNCIONAL.md refleje el cambio, el historial de cambios esté anotado y la consistencia código<->documentación esté verificada. Si el código cambia, la documentación cambia en la misma tarea.
